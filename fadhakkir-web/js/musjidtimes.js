...
for (let i = 0; i < 10; i++) {
	document.querySelectorAll('.prayer-times .box')[i].addEventListener('click', () => {
		...
		if (!(document.querySelector('.pop-up-box').classList.contains('hidden-4'))) {
			document.querySelector('.pop-up-box').style.setProperty('--animation-startvar' ,`calc(${document.querySelectorAll('.prayer-times .box')[i].offsetTop - document.querySelector('.prayer-times').scrollTop}px + 58vw - 10dvh)`)
...
const init_popup = function() {
	for (let i = 0; i < 10; i++) {
		document.querySelectorAll('.prayer-times .box')[i].addEventListener('click', async () => {
			document.querySelector('.musjids-loader').style.opacity = '1';
			if (i === 1 || i === 4 || i === 5 || i === 8 || i === 7 || i === 10) {
				...
				//hiding musjid section
				document.querySelector('.pop-up-musjids').classList.add('hidden-3');
				document.querySelector('.musjids-loader').style.opacity = '0';
			}
		})
	}
}
...
//hiding pop-up-musjid if not in newcastle
const hideMusjidTimes = async function () {
